...
- `reconcileEntraDsiDrift`:
  - Default Timer: 3AM on every Monday.
  - Description: Detects Entra/DSI account drift in both directions (DSI internal users that never linked to a matching Entra account, and recently created Entra accounts with no matching DSI user) and reports anomalies via the audit log. Read-only — does not make any automated changes.
...
```json
{
  ...
  "Values": {
    ...
    "TIMER_RECONCILE_ENTRA_DSI_DRIFT": "0 0 3 * * 1",
...
| TIMER_RECONCILE_ENTRA_DSI_DRIFT | The NCronTab expression that sets the schedule for the `reconcileEntraDsiDrift` function (`./src/functions/reconcileEntraDsiDrift`). | Read the [documentation on NCrontab formatting](https://learn.microsoft.com/en-us/azure/azure-functions/functions-bindings-timer?tabs=python-v2%2Cisolated-process%2Cnodejs-v4&pivots=programming-language-typescript#ncrontab-expressions) and use a [tester](https://ncrontab.swimburger.net/) to verify your expression runs as you'd expect. Deliberately offset from `TIMER_REMOVE_GENERATED_TEST_ACCOUNTS` (also Monday) so this job doesn't scan the same recently-created Entra population while that job is still mid-cleanup. | `0 0 3 * * 1` Runs at 3AM UTC on every Monday. |
