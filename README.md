...
- `deleteDeactivatedAccounts`:
  ...
  - Description: Permanently deletes accounts deactivated for 12+ months, from the directories database, organisation/service associations, and Entra ID. Eligibility uses the latest `user_status_change_reasons` row, falling back to `FEATURE_SHIP_DATE_DELETE_DEACTIVATED_ACCOUNTS` for accounts with no such row. Runs in dry-run mode (audits candidates, deletes nothing) unless `DRY_RUN_DELETE_DEACTIVATED_ACCOUNTS` is explicitly `"false"`, and processes at most `DELETE_DEACTIVATED_ACCOUNTS_BATCH_CAP` candidates per run.
...
```json
{
  ...
  "Values": {
    ...
    "FEATURE_SHIP_DATE_DELETE_DEACTIVATED_ACCOUNTS": "2020-01-01",
...
| FEATURE_SHIP_DATE_DELETE_DEACTIVATED_ACCOUNTS | The eligibility date used for accounts with no `user_status_change_reasons` row (see NSA-9963/NSA-9964) — an ISO date string, e.g. `2026-09-01`. In production this must be set to the function's actual go-live date; for local testing, set it in the past so fallback-eligible accounts show up as candidates. | Set to any past date to exercise the fallback path locally. | `""` |
