...
export async function removeUnresolvedInvitations(
  ...
  try {
    ...
    context.info(
      `removeUnresolvedInvitations: ${invitationIds.length} invitations found`,
    );
    ...
    for (let index = 0; index < invitationIds.length; index += batchSize) {
      ...
      context.info(
        `removeUnresolvedInvitations: Removing invitations ${invitationRange}`,
      );
      ...
      if (errored.count > 0) {
        errored.errors.forEach((error) =>
          context.error(`removeUnresolvedInvitations: ${error}`),
        );
      }
