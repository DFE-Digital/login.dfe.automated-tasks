 ...
 */
async function getUserApiRecords(apis: apiClients, userId: string, correlationId: string): Promise<userApiRecords> {
  ...
  return {
    services: (serviceRecordsResult as PromiseFulfilledResult<userServiceRecord[]>).value,
    organisations: (organisationRecordsResult as PromiseFulfilledResult<userOrganisationRecord[]>).value,
  }
};
...
async function deleteUserApiRecords(apis: apiClients, userId: string, apiRecords: userApiRecords, correlationId: string): Promise<actionResult<string>> {
  ...
  const servicesDeletedResults = await Promise.allSettled(services.map(async (record) =>
    apis.access.deleteUserService(userId, record.serviceId, record.organisationId, correlationId)
  ));
  const organisationsDeletedResults = await Promise.allSettled(organisations.map(async (record) =>
    apis.organisations.deleteUserOrganisation(userId, record.organisation.id, correlationId)
  ));
 ...
 */
export async function removeGeneratedTestAccounts(_: Timer, context: InvocationContext): Promise<void> {
  try {
    ...
    context.info(`removeGeneratedTestAccounts: ${userIds.length} users and ${invitationIds.length} invitations found`);
    ...
    for (let index = 0; index < userIds.length; index += batchSize) {
      ...
      const { successful, failed, errored } = filterResults(await Promise.allSettled(batch.map(async (userId) => {
        const apiRecords = await getUserApiRecords(apis, userId, correlationId);
        return deleteUserApiRecords(apis, userId, apiRecords, correlationId);
      })));
      ...
      if (errored.count > 0) {
        errored.errors.forEach((error) => context.error(`removeGeneratedTestAccounts: ${error}`));
      }
    ...
    for (let index = 0; index < invitationIds.length; index += batchSize) {
      ...
      context.info(`removeGeneratedTestAccounts: Removing invitations ${invitationRange}`);
